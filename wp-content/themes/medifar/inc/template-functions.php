...
function custom_override_checkout_fields($fields)
{
	...
	$fields['billing']['cuil'] = array(
		'label'     => __('Nro. CUIT / CUIL', 'woocommerce'),
		'placeholder'   => _x('Informe su número de CUIT / CUIL', 'placeholder', 'woocommerce'),
		'required'  => true,
		'class'     => array('form-row-wide'),
		'clear'     => true
	);
...
function matricula_update_order_meta($order_id)
{
	...
	if (!empty($_POST['cuil'])) {
		update_post_meta($order_id, 'cuil', esc_attr($_POST['cuil']));
	}
}
...
function matricula_display_admin_order_meta($order)
{
	...
	echo '<p><strong>' . __('CUIT / CUIL') . ':</strong> ' . get_post_meta($order->get_id(), 'cuil', true) . '</p>';
}
...
add_action(
	...
	'cuotas_display_cust_order_meta',
	10,
	1
);
...
add_action(
	'woocommerce_order_details_after_order_table',
	'total_financiado_display_cust_order_meta',
	10,
	1
);
...
function misha_add_email_order_meta($order_obj, $sent_to_admin, $plain_text)
{
	...
	if ($plain_text === false) {
		...
		echo '<p><strong>' . __('CUIT / CUIL') . ':</strong> ' . get_post_meta($order_obj->get_id(), 'cuil', true) . '</p>';
	} else {
